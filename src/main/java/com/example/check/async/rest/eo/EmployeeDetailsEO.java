...
public class EmployeeDetailsEO {
	...
	@Async("asyncExecutor")
	public CompletableFuture<AddressVO> getEmployeeAddress() throws InterruptedException, RestClientException {
		LOGGER.info("Inside getEmployeeAddress --------" + Instant.now());
		LOGGER.info("Current Thread : {}", Thread.currentThread().getName() + Thread.currentThread().getId());
		Thread.sleep(3000L);
	...
	public CompletableFuture<DepartmentVO> getEmployeeDept() throws InterruptedException {
		...
		LOGGER.info("Current Thread : {}", Thread.currentThread().getName() + Thread.currentThread().getId());
		Thread.sleep(2000L);
		DepartmentVO dept = restTemplate.getForObject("http://localhost:11401/employee/address", DepartmentVO.class);
		LOGGER.info("{} is complete" + Instant.now(),
				Thread.currentThread().getName() + Thread.currentThread().getId());
